<template>
  <div
    ...
    v-if="!filterTableStore.isEmpty"
  >
    <div
      :style="gridHead"
      class="bold absolute top-0 grid w-full items-center justify-center bg-white p-4 text-2xl uppercase"
    >
      <input
        type="checkbox"
        name=""
    ...
    <div class="mt-20">
      ...
      >
        <input
          ...
          id=""
          :checked="checkAll"
          class="ml-4 h-8 w-8 text-brand-dark"
        />
        <RouterLink
          :to="{ name: route, params: { id: item.id || item.user_id } }"
          :style="gridTable"
          class="main-grid my-2 grid w-full items-center justify-center p-4 px-4 text-left text-xl text-brand-dark"
        >
          <p></p>
          <p>{{ item[headings[0]] || "" }}</p>
          <p>{{ item[headings[1]] || "" }}</p>
          ...
          <p>
            ...
            <span v-else-if="item.verified">
              <span
                :class="statusStyles(item.verified)"
                class="flex w-32 items-center justify-center rounded p-2 capitalize text-white"
              >
                {{
